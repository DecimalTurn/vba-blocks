...
A project consists of all the relevant information needed for working with vbapm projects/packages, including the manifest, workspace, and resolved dependencies.

1. The `vba_package.toml` manifest is loaded in the current working directory
...
- Cache: `{user}/.vbapm` includes registry -> packages -> sources (cache for each stage of resolve and fetch)
...
5. Import the staged `BuildGraph` into the previously created binary from the vbapm addin
