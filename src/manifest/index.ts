...
/*
  ...
  The parsed vba_package.toml manifest.
...
const EXAMPLE = `Example vba_package.toml for a package (e.g. library to be shared):
...
Example vba_package.toml for a project (e.g. workbook, document, etc.):
...
export async function loadManifest(dir: string): Promise<Manifest> {
	...
	if (!(await pathExists(file))) {
		throw new CliError(
			...
			dedent`
        vba_package.toml not found in "${dir}".
...
export async function writeManifest(manifest: Manifest, dir: string) {
	...
	const path = join(dir, "vba_package.toml");
	let toml: string;
